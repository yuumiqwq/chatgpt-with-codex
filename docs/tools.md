...
chatgpt-with-codex exposes durable Codex work, temporary agent execution and optional host operations. See [work management](work-management.md) for lifecycle, default full access and retention semantics.
...
| list_codex_projects | Optional query/codex_home/cursor/scan_limit/archived; project groups from a bounded history page. |
| list_codex_threads | Optional query/cwd/codex_home/cursor/limit/archived; native IDs and titles. |
...
Codex RPC rejections expose `error.rpc_method`, numeric `error.rpc_error_code` and `error.rpc_error_category`. `CODEX_THREAD_BUSY` with category `thread_busy` identifies the allowlisted `-32600` / `thread <uuid> already has an active writer` conflict; wait for the other writer to release that thread before continuing it. Other valid RPC errors use `CODEX_RPC_ERROR` with category `unknown`, including unrelated `-32600` errors. Messages are fixed safe text; raw server messages, error data and stderr are omitted. These fields also survive stored result/registry reads and steering error serialization. `CODEX_PROTOCOL_ERROR` is reserved for malformed or unexpected protocol messages/structure; an unanswered RPC deadline uses `CODEX_EXECUTION_FAILED`. No automatic retry or thread replacement is performed.

DSH supports interruption but not steering. `control_task` must reach the Bridge process that owns the live executor; a different process can read stored status but cannot steer that executor. `WORK_RESULT_WRITE_FAILED` indicates that execution ended but its result could not be persisted; check files and storage before retrying.

All tools exchange JSON text through MCP. Obtain IDs from discovery/creation responses, and use one project selector per call to avoid relying on implementation precedence. The complete input schemas are returned by MCP `tools/list`; the repository's `src/work-tools.ts`, `src/host/host-tools.ts` and `src/mcp-stdio.ts` define them.
